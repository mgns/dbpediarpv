...
def readVersions():
	#datasets = json.loads('{"version" : [ { "id" : "3.0" , "files" : ["http://downloads.dbpedia.org/3.0/en/infoboxproperties_en.nt.bz2", "http://downloads.dbpedia.org/3.0/en/articles_label_en.nt.bz2", "http://downloads.dbpedia.org/3.0/en/externallinks_en.nt.bz2"] } ] }')
	versions = []
...
def processFile(fsrcpath, vdestpath, rfile):
	...
	for line in lines:
		...
		try:
			# ignore empty lines
			if line.strip():
				f, content = processTriple(line.decode(encoding='latin_1',errors='strict'), vdestpath, rfile)
				if (not f is None):
					filecontents[f].add(content)
		except BaseException as e:
			logging.error("--- Error while processing " + fsrcpath + " line: " + str(i) + " / " + line.decode(encoding='latin_1',errors='strict') + " : " + str(e))
			pass
...
def processTriple(triple, path, rfile):
	...
	spo = triple.split(" ")
	s, p , o = spo[0], spo[1], " ".join(spo[2:-1])
